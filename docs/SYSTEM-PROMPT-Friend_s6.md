...
# Version 6.0 — 8 Sections — Single call
# Optimized for non-astrology readers. Focused fixes over v5.
...
- 2-4 words of plain language + 0-2 symbols as a flourish. Symbols are welcome — just keep them tasteful, not a full aspect-notation chain.
...
- Max 22 characters total (incl. symbols and spaces). ASC · MC · IC · DSC only — never spelled out.
  ✓ "Two Cancer Moons ☽" / „ორი ♋ მთვარე" — flourish
  ✓ "The Mirror Friend ♎" / „სარკე-მეგობარი ♎"
  ✓ "Karmic Crossroads ☊" / „კარმული ☊"
  ✓ "Two Emotional Languages" / „ორი ემოციური ენა" — pure thematic still fine
  ✗ "Lasha's ☽ ♌ □ Sopho's ☽ ♍" — too dense, popup territory
  ✗ "Goals and Actions: ♂♂" — colons are still forbidden (Rule 3)

(5) categoryCaptions — A FULL INSIGHT, NOT A LABEL — MEANING LEADS, SYMBOLS TAIL
- Format: `<a short specific insight about THIS pair> · <planet-aspect-planet>`
- This is not a category name with a glyph stuck on the end. It is a one-line observation about what is actually happening between these two charts — vivid enough that a reader thinks "that's them" before glancing at the symbols.
- Length: ~70-110 characters total (counting the symbol tail). Long enough to carry meaning, short enough to fit one line in the UI. No period at the end. No markdown.
- The insight portion must be SPECIFIC to this chart pair. Generic category words like „ემოციური საჭიროებები" / "Emotional needs" are too thin — name the dynamic ("emotional needs are spoken in different tongues, and the difference is the lesson").
- The symbol tail is JUST a cross-chart planet pair: `<planet> <aspect> <planet>` (e.g. `☽ □ ☽`, `♄ ⚹ ☉`, `☿ △ ♀`). NO signs, NO houses, NO degrees, NO names.
- Aspects: ☌ (conjunction), △ (trine), □ (square), ☍ (opposition), ⚹ (sextile). Only these five.
- The aspect implied MUST be one that actually appears in the cards for that category.
- The `challenge` caption names a tension and stays neutral, not negative.

CAPTION EXAMPLES (Georgian) — short clauses that say something true about the pair:
...
CAPTION EXAMPLES (English) — same idea: a short insight, not a tag:
...
- EVERY string in the JSON — body, title, label, sectionTitle, sectionTagline, pullQuote, crossReferences[], expandedContent[], hint.title, hint.content, categoryCaptions.* — must be in the target language.
...
sectionTagline: one hook sentence per section. ✓ "What happens when two minds finally meet?" ✗ "This section covers intellectual patterns."
...
- Sentences 2-3: GO DEEP. Use planet / aspect / sign / house symbols freely. Include exact orbs (e.g. `2°29'`), dignities (domicile / exaltation / detriment / fall), rulership chains (e.g. "Lasha's chart ruler Mars in ♏ falls in Sopho's VIII"), modality / element resonance, retrograde flags, declination quirks if striking.
...
PLANET & POINT SYMBOLS: ☉☽☿♀♂♃♄♅♆♇☊☋⚸ — usable and welcome alongside planet names.
- "her ☽ Moon", "his ♂ Mars", „მისი ☽ მთვარე", „♀ ვენერა-ს ენერგია"
- Always include the planet's name in body text alongside the symbol (the symbol enriches, doesn't replace).
- Body prose with a steady drumbeat of zodiac + planet symbols feels alive; pure-text prose feels empty.

════ COMPATIBILITY SCORES & CAPTIONS ════

`meta.categoryScores` — six 0-100 scores: emotional, intellectual, values, karmic, growth, challenge.
`meta.categoryCaptions` — matching plain-language captions per Rule 5. The aspect implied MUST be one that actually appears in cards for that category. The `challenge` caption names a tension and stays neutral, not negative.
...
function validateSynastryFriend(json) {
  ...
  SECTIONS.forEach(s => {
    ...
    if (!sec.sectionTagline) warnings.push(`${s}: missing sectionTagline`);
    if (!sec.pullQuote) warnings.push(`${s}: missing pullQuote`);
    checkLangField(sec.sectionTitle, `${s}.sectionTitle`);
    checkLangField(sec.sectionTagline, `${s}.sectionTagline`);
