...
══════════════ ARRAY FORMATTING — MANDATORY ══════════════

These rules govern how `body` and `expandedContent` arrays are structured. The renderer reads each element individually — inline lists silently break the UI layout.

─── body[] ───

Each element is ONE of:
  a) A prose paragraph string (default)
  b) A single numbered list item: `"1. **Title**: body text"`

RULES:
- Each numbered item MUST be its own array element — NEVER combine multiple items in one string
- ALWAYS use `1. ` format (numeral · period · space) — NEVER `1)` parenthesis format
- NEVER embed a list inline within a prose paragraph

  ✓ CORRECT:
  "body": [
    "intro prose paragraph",
    "1. **გაცნობიერება:** პირველი ნაბიჯია...",
    "2. **მიღება:** შემდეგი ნაბიჯია...",
    "3. **ინტეგრაცია:** ბოლო ნაბიჯია..."
  ]

  ✗ WRONG — inline list in one string:
  "body": ["პრაქტიკა მოიცავს: 1) გაცნობიერებას, 2) მიღებას, 3) ინტეგრაციას"]

  ✗ WRONG — parenthesis format:
  "body": ["1) გაცნობიერება: ...", "2) მიღება: ..."]

─── expandedContent[] ───

Each element is ONE of three types — always its own array element:
  a) Section header:  `"**Title:**"` — bold phrase ending with colon, standalone
  b) Numbered item:   `"1. **Title**: body text"` — each item is a separate element
  c) Prose paragraph

RULES:
- Consecutive numbered items auto-render as a two-column table (gold title | body text)
- Section headers become decorative dividers with dot ornaments
- NEVER inline multiple steps in one string
...
Card: {
  ...
  "title": "string",
  "body": [
    // Each element is a prose paragraph OR a single numbered item "1. **Title**: text"
    // NEVER: "intro: 1) a, 2) b" — each numbered item must be its own element
    // NEVER: "1) " format — always "1. " (period, not parenthesis)
    "paragraph"
  ],
  "crossReferences": ["short astrological context: degrees, dignity, orbs, rulerships"],
  "expandedContent": [
    // Each element is one of:
    //   "**Section Header:**"          → renders as decorative intro divider
    //   "1. **Title**: body text"      → renders as two-column table row (gold | body)
    //   "prose paragraph"              → renders as plain paragraph
    // Consecutive numbered items automatically form the table. Never collapse multiple into one string.
    "paragraph"
  ] | null,
...
# CHANGELOG
- i11: Locked `body[]` and `expandedContent[]` array formatting. Numbered list items must be separate array elements using `1. **Title**: text` format. Inline `1) a, 2) b` prose lists are forbidden. Ensures reactive renderer always produces counter list (body) and two-column table (expanded).
