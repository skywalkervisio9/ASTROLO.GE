...
4. MAJOR ASPECTS (all with orb < 6°) — MANDATORY, never skip this section
...
- Max ~20 characters. Keep it tight.
- NEVER a chain of glyphs or a wall of notation. That is popup territory, not the badge.
  ✗ „☉ ♎ 11°25' — VIII ☌ ☽ □ ♂" (glyph chain)   ✗ „☉ მზე" (never the planet NAME beside its symbol)
...
ANGLES — STRICT FORMATTING (MANDATORY):
  ...
  ✗ "Ascendant", "Midheaven", "Imum Coeli", "Descendant" — never use these forms in body text, labels, or titles.
...
ANGLES — STRICT FORMATTING (MANDATORY):
  ...
  ✗ „ასცენდენტი", „დესცენდენტი", „Ascendant", „Descendant", „Midheaven", „Imum Coeli" — never use these forms anywhere in body text, labels, or titles.

BORROWED TERMS: Acceptable in parentheses: „flow state"

NEVER TRANSLITERATE: ✗ „ტაიტ" → ✓ „ზუსტი" | ✗ „ესკაპიზმი" → ✓ „გაქცევა"
COMMON TERMS IN GEORGIAN (never English in body): „natal chart" → „ნატალური რუკა" / „რუკა" | „aspect" → „ასპექტი" | „house" → „სახლი" | „placement" → „პოზიცია"
...
SPELLING: Proofread common connectives and function words — write „რადგან" (not „ღრადგან"), „რომ", „რომელიც", „თუმცა", „მაგრამ" correctly. A single wrong letter in a frequent word is jarring; reread each paragraph before finalizing.
...
AspectInterp: {
  "planet1": "string",  // ALWAYS English: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto, North Node, South Node, Lilith, Chiron
  "planet2": "string",  // ALWAYS English — even in Georgian readings
  "aspect": "string",   // ALWAYS English: conjunction, trine, square, opposition, sextile
  "interpretation": "1-2 sentence psychological interpretation in the target language — specific to this chart",
}

aspectInterpretations rules (MANDATORY — this array MUST always be present and populated):
- NEVER omit aspectInterpretations. An empty array [] is a critical error. Every reading must include aspect interpretations.
